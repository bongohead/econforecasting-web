...
document.addEventListener('DOMContentLoaded', function() {
	...
	const get_cleaned_hist = get_hist_obs.then(function(r) {
		...
		const hist_values_raw = r.map(x => {
			const ttm = parseInt(x.varname.substring(2, 4)) * (x.varname.substring(4, 5) === 'm' ? 1 : 12);
			return x.data.map(y => ({
		...
		}).flat().sort((a, b) => a.unixdate > b.unixdate).filter(a => a.unixdate >= dayjs('2010-11-01')); // Mismatch! 5-10-30 yields exist from 2007, but remainder only exist from 10/1/10
	...
	Promise.all([get_cleaned_hist, get_cleaned_forecast]).then(function([{hist_values, hist_vdate}, {forecast_values, forecast_vdate}]) {
		...
		setData('real-treasury-curve', res);
		return(res);
	})
	.then(function(res) {
	...
	$('#chart-container-1').on('click', '#treasury-curve-subtitle-group > button.treasury-curve-action-button', function() {
		...
		setData('real-treasury-curve', {...getData('real-treasury-curve'), ...{play_state: newPlayState, play_index: newPlayIndex}});
...
function drawChart(treasury_data, play_index, forecast_vdate, hist_vdate, site) {
	...
	const o = {
		...
		xAxis: {
			...
			tickPositions: [3, 6, 12, 24, 60, 84, 120, 240, 360],
			breaks: [{
				from: 12,
				to: 12 + (24 - 12) * .2
			}, {
			...
			series: {
				...
				point: {
					events: {
						click: function () {
							'real-treasury-curve',
							setData('real-treasury-curve', {
								...getData('real-treasury-curve'),
...
function updateChart() {
	let ud = getData('real-treasury-curve');

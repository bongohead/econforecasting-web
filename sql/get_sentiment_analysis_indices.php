...
$index_values = $sql -> select("
	SELECT
		index_id, date, count, ROUND(score + adjustment, 1) AS score, ROUND(score_7dma + adjustment, 1) AS score_7dma, DATE(sentiment_analysis_index_values.created_at) AS created_at
...
$index_stats = $sql -> select("
	...
	FROM 
		...
		SELECT
			...
			score_7dma + adjustment as last_val,
			score_7dma - lag(score_7dma, 7) OVER (PARTITION BY index_id ORDER BY DATE) AS ch1w,
			score_7dma - lag(score_7dma, 30) OVER (PARTITION BY index_id ORDER BY DATE) AS ch1m
		FROM sentiment_analysis_index_values
		INNER JOIN sentiment_analysis_indices ON sentiment_analysis_index_values.index_id = sentiment_analysis_indices.id
		WHERE index_id IN (1, 2, 3, 4)
		) a
